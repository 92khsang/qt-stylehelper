...
class TestStyleSheetExporter(unittest.TestCase):
    ...
    @patch(
        "qt_stylehelper._stylesheet.Path.mkdir", side_effect=Exception("Mkdir error")
    )
    def test_export_mkdir_error(self, mock_mkdir):
        with self.assertRaises(Exception):
            StyleSheetExporter.export(
                stylesheet="body {}", destination_dir="destination"
            )
    ...
    def test_generate_qrc_file_with_files(self, mock_file):
        with (
            patch.object(
                Path, "iterdir", return_value=[Path("file1.svg"), Path("file2.svg")]
            ) as mock_iterdir,
