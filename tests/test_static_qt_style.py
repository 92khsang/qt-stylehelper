...
class TestStaticQtStyleTools(unittest.TestCase):
    ...
    @patch("qt_stylehelper.StaticThemeDirectoryStructure")
    def test_manual_init_with_valid_structures(
        self, mock_static_theme_directory_structure
    ):
        ...
        self.style_tools._convert_to_dict = MagicMock(
            return_value={"mock_theme": mock_theme_struct}
        )
    ...
    def test_apply_stylesheet_valid_theme(self, mock_qt_handler):
        ...
        with patch("sys.modules", {"PySide6": MagicMock()}):
            ...
            self.style_tools.get_theme_list = MagicMock(
                return_value=["default", "dark"]
            )
            ...
            mock_qt_handler.apply_stylesheet.assert_called_with(
                widget_mock, "mock_stylesheet"
            )
            mock_qt_handler.add_search_paths.assert_called_with(
                resource_dir="/mock/icons"
            )
    ...
    def test_is_rendered_from_builtin_template(self):
        ...
        with patch("builtins.open", mock_open(read_data="mock data")):
            result = self.style_tools.is_rendered_from_builtin_template(
                Path("mock.qss")
            )
        ...
        with patch("sys.modules", {"PySide6": MagicMock()}):
            ...
            with self.assertRaises(ValueError):
                try:
                    ...
                    self.fail(
                        "apply_stylesheet() raised RuntimeError unexpectedly when initialized."
                    )
    ...
    def test_scan_theme_dir(self, mock_path):
        ...
        mock_theme_parent_dir.iterdir.return_value = [
            mock_theme_dir_1,
            mock_theme_dir_2,
        ]
        ...
        self.assertTrue(
            all(isinstance(struct, StaticThemeDirectoryStructure) for struct in result)
        )
    ...
    def test_filter_and_proceed_structures(
        self,
        mock_is_structure_valid,
        mock_proceed_structures,
        ...
        result = self.style_tools._filter_and_proceed_structures(
            [invalid_struct, valid_struct]
        )
    ...
    def test_advanced_filter_structures(self, mock_generate_regex, mock_is_rendered):
        ...
        valid_qss_file.open.return_value.__enter__.return_value.read.return_value = (
            "/* Valid QSS #FFFFFF #000000 */"
        )
        ...
        # Call the method
        result = self.style_tools._advanced_filter_structures(
            [valid_struct, invalid_struct]
        )
