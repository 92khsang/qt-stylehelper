...
class RequireInitTest(unittest.TestCase):
    def test_require_init(self):
        ...
        self.assertEqual(
            str(context.exception), "test_method cannot be called unless initialized."
        )
...
class RequireInitForAllMethodsTest(unittest.TestCase):
    def test_require_init_for_all_methods(self):
        ...
        with self.assertRaises(RuntimeError) as context:
            instance.method1()
        self.assertEqual(
            str(context.exception), "method1 cannot be called unless initialized."
        )

        with self.assertRaises(RuntimeError) as context:
            instance.method2()
        self.assertEqual(
            str(context.exception), "method2 cannot be called unless initialized."
        )
    ...
    def test_require_init_for_all_methods_with_inheritance(self):
        ...
        self.assertEqual(
            str(context.exception), "base_method cannot be called unless initialized."
        )

        with self.assertRaises(RuntimeError) as context:
            instance.child_method()
        self.assertEqual(
            str(context.exception), "child_method cannot be called unless initialized."
        )
...
class OverrideTest(unittest.TestCase):
    ...
    def test_invalid_override(self):
        ...
        with self.assertRaises(AttributeError) as context:
            ...
            instance.new_method()
        self.assertIn(
            "does not override any method in the base class", str(context.exception)
        )
